...
class hqDriver():
                                    ...
                                    while k < len(df_script):
                                     if df_script.loc[k][sTestCaseName]==tcName:
                                      ...
                                      if not objnameexists:
                                       ...
                                       if ifelseflag == 0:
                                            print("qwertyui")
                                            result,actualResult=hqDriver.selectAction(actionName,stepData,None,None,None,stepdataName,testDataExcel,outputSheet,browser,k)
                                       else:
                                           if r=='pass':
                                            result='pass'
   ...
   #Method for calling Java Action Functions
    def selectAction(stepaction,stepdata,objname,objprop,objval,OutputHeader,testDataExcel,sheet,browser,k):
        if str(stepdata)=='nan':
            stepdata=""
        try:
            ...
            if str(word[1]):
                if OutputHeader not in df.columns:
                    ...
                    ActionClasses.WriteToParticularCellInExcel.write(testDataExcel, sheet, 2, l + 1, word[1])
                else:
                    ActionClasses.WriteToParticularCellInExcel.write(testDataExcel, sheet, 2,
    ...
    def create_Word_Doc_Result(screenShotList,screenShotDetails,iter,tcname,reqname,execStart,df):
        ...
        #wdFormatPDF=17
        #doc.SaveAs(hqDriver.resultDir + '\\Reports\\'+iter+'.pdf', FileFormat=wdFormatPDF)
        #doc.Close()
        #word.Quit()
    ...
    def get_data_from_excel_sheet(excel,sheet,columnname,rowno):
        if excel:
            ...
            try:
                stepData = str(df.loc[0][columnname])
                return stepData
            except Exception as e:
                print(e)
