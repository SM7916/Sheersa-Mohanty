...
public class DownloadNBVfilesFromServer {
	...
	public String getFile(String FTPDetails) throws JSchException, SftpException {
		try {
			...
			if (FTPDetails.contains("#")) {
				...
				String ThreadCount = Details.split("#")[9];
				String BatchBusinessDate = Details.split("#")[10]; // its in
																	// MM-dd-yyyy
				...
				if (batchcode.equalsIgnoreCase("CMNBVD3B")) {
					Browser.Driver.switchTo().frame("BJP");
					// Update File Path
					Browser.Driver.findElement(By.id("BJP:15$BATCH_PARM_VAL")).click();
					Browser.Driver.findElement(By.id("BJP:15$BATCH_PARM_VAL")).clear();
					Browser.Driver.findElement(By.id("BJP:15$BATCH_PARM_VAL")).sendKeys(copyFrom);
					Browser.Driver.findElement(By.id("BJP:16$BATCH_PARM_VAL")).click();
					Thread.sleep(10000);
				}

				else if (batchcode.equalsIgnoreCase("CMNBVD2B")) {
					Browser.Driver.switchTo().frame("BJP");
					// Update File Path
					Browser.Driver.findElement(By.id("BJP:9$BATCH_PARM_VAL")).click();
					Browser.Driver.findElement(By.id("BJP:9$BATCH_PARM_VAL")).clear();
					Browser.Driver.findElement(By.id("BJP:9$BATCH_PARM_VAL")).sendKeys(copyFrom);
					Browser.Driver.findElement(By.id("BJP:10$BATCH_PARM_VAL")).click();
					Thread.sleep(10000);
				}
				...
				try {
					 ...
					 * 
					 * //parse the string into Date object Date date =
					 * sdfSource.parse(BatchBusinessDate);
					 * 
					 * //create SimpleDateFormat object with desired date format
					 * SimpleDateFormat sdfDestination = new
					 * SimpleDateFormat("yyyy-MM-dd");
					 * 
					 * //parse the date into another format BatchBusinessDate =
					 * sdfDestination.format(date);
					 * 
					 * System.out.
					 * println("Date is converted from dd-MM-yyyy format to yyyy-MM-dd"
					 * ); System.out.println("Converted date is : " +
					 * BatchBusinessDate); String
					 * BBDate=BatchBusinessDate.replaceAll("-", "");
					 */

					String fileNameformat1 = "FORMERGE-i1501a_4527a-M-%s(\\d{6})%s\\d{7}%s.in";
					String fileNamePattern1 = String.format(fileNameformat1, batchcode, BatchNumber, ThreadCount);

					String fileNameformat2 = "FORMERGE-i1502a_4698a-M-%s(\\d{6})%s\\d{7}%s.in";
					String fileNamePattern2 = String.format(fileNameformat2, batchcode, BatchNumber, ThreadCount);

					String fileNameformat3 = "FORMERGE-i1503a_4701a-M-%s(\\d{6})%s\\d{7}%s.in";
					String fileNamePattern3 = String.format(fileNameformat3, batchcode, BatchNumber, ThreadCount);

					String fileNameformat4 = "FORMERGE-i1504a_4525a-M-%s(\\d{6})%s\\d{7}%s.in";
					String fileNamePattern4 = String.format(fileNameformat4, batchcode, BatchNumber, ThreadCount);

					String fileNameformat5 = "FORMERGE-i1505a_4700a-M-%s(\\d{6})%s\\d{7}%s.in";
					String fileNamePattern5 = String.format(fileNameformat5, batchcode, BatchNumber, ThreadCount);

					System.out.println("After appending date file name = " + fileNamePattern1);

					for (int i = 0; i < filelist.size(); i++) {
						System.out.println(filelist.get(i).toString());
						System.out.println("fILEPattern to Matched = " + fileNamePattern1);
						...
						if (temp.matches(fileNamePattern1) || temp.matches(fileNamePattern2)
								|| temp.matches(fileNamePattern3) || temp.matches(fileNamePattern4)
								|| temp.matches(fileNamePattern5)) {
							System.out.println("file is found");

							sftpChannel.get(copyFrom + temp, copyTo + temp, monitor, ChannelSftp.OVERWRITE);
							System.out.println("file is found at: " + copyFrom + temp + " and copied to destination "
									+ copyTo + temp);
